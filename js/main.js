...
<span class="price">${vnd(infoProduct.price)}</span>
    ...
    // Chỉ hiện nút mua nếu KHÔNG phải Admin
    if (!isAdmin) {
        modalHtml += `<div class="modal-footer-control">
    ...
    // Chỉ gán sự kiện click mua hàng nếu nút đó tồn tại (Không phải Admin)
    if (!isAdmin) {
        let productbtn = document.querySelector('.button-dat');
        if (productbtn) { // Kiểm tra null để tránh lỗi
...
// Them SP vao gio hang (ĐÃ SỬA LỖI BIẾN)
function addCart(index) {
    ...
    // Sửa lỗi: dùng currentuser (chữ thường) và productcart.id
    ...
    if (fullNameUser && phoneUser && passwordUser && passwordConfirmation && checkSignup) {
        if (passwordConfirmation == passwordUser) {
            ...
            if (!checkloop) {
                ...
                // --- ĐÃ SỬA: KHÔNG AUTO LOGIN ---
                toast({ title: 'Thành công', message: 'Tạo thành công tài khoản !', type: 'success', duration: 3000 });
                // closeModal(); // Không đóng modal để chuyển tab
    ...
    if (phonelog && passlog) {
        ...
        if (vitri == -1) {
            ...
            if (accounts[vitri].status == 0) {
                ...
                closeModal();
window.location.reload(); // Reload để cập nhật giao diện Admin/User
            }
        } else {
...
function kiemtradangnhap() {
    let currentUser = localStorage.getItem('currentuser');
    let cartIcon = document.querySelector('.header-middle-right-item.open[onclick="openCart()"]'); // Lấy nút giỏ hàng
    ...
    if (currentUser != null) {
        ...
        // Kiểm tra loại tài khoản (0: Khách hàng, 1: Admin)
        if (user.userType == 1) {
            // === NẾU LÀ ADMIN ===

            // 1. Ẩn biểu tượng giỏ hàng
            if (cartIcon) cartIcon.style.display = 'none';

            // 2. Tạo Menu cho Admin (Không có 'Đơn hàng đã mua')
            menuHtml += `<li><a href="./admin.html"><i class="fa-light fa-gear"></i> Quản lý cửa hàng</a></li>`;
            ...
            // 1. Hiện biểu tượng giỏ hàng
            if (cartIcon) cartIcon.style.display = 'block';

            // 2. Tạo Menu cho Khách (Có 'Đơn hàng đã mua')
            menuHtml += `<li><a href="javascript:;" onclick="myAccount()"><i class="fa-light fa-circle-user"></i> Tài khoản của tôi</a></li>`;
            menuHtml += `<li><a href="javascript:;" onclick="orderHistory()"><i class="fa-regular fa-bags-shopping"></i> Đơn hàng đã mua</a></li>`;
            menuHtml += `<li class="border"><a id="logout" href="javascript:;"><i class="fa-light fa-right-from-bracket"></i> Thoát tài khoản</a></li>`;
        }

        // Render menu ra màn hình
        ...
        // Nếu chưa đăng nhập thì hiện giỏ hàng bình thường (để khách vãng lai xem)
        if (cartIcon) cartIcon.style.display = 'block';
    }
}
...
function logOut() {
    ...
    // Lưu lại giỏ hàng của user trước khi đăng xuất
...
for (let i = 0; i < user.cart.length; i++) {
    ...
    // Chuyển hướng về trang chủ (sẽ tự động reset lại giao diện như chưa đăng nhập)
...
function renderProducts(showProduct) {
    ...
    if (showProduct.length == 0) {
        ...
        showProduct.forEach((product) => {
            ...
            // Nếu KHÔNG phải Admin thì mới hiện nút Đặt hàng
            if (!isAdmin) {
...
// Find Product
var products = JSON.parse(localStorage.getItem('products')) || []; // Sửa lỗi null
...
// Khởi chạy khi trang tải xong (Đã gộp)
window.onload = function () {
    kiemtradangnhap();
    updateAmount();
    updateCartTotal();

    // An toàn hơn với check null
    let products = JSON.parse(localStorage.getItem('products')) || [];
